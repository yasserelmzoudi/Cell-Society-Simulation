...
In order to add an additional language, a corresponding </code>.properties</code> file in lowercase must be made in the LanguageResources package. Once the file is made, all the keys found in the </english.properties</code> must be copied and have their values changed accordingly. An example of the </code>spanish.properties</code> file can be seen below. 
